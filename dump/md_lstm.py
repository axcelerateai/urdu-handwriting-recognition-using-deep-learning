...
class MultiDimensionalLSTMCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        c1, c2, h1, h2 = state 
        prod = slim.fully_connected(tf.concat([inputs, h1, h2], axis=1), 5*H, activation_fn=None)
        ...
        i = tf.nn.sigmoid(ln(i, scope='i/'))
        f1 = tf.nn.sigmoid(ln(f1, scope='f1/'))
        f2 = tf.nn.sigmoid(ln(f2, scope='f2/'))
        g = tf.nn.tanh(ln(g, scope='g/'))
        ...
        o = tf.nn.sigmoid(ln(o, scope='o/'))

        next_hidden_state = o*tf.nn.tanh(ln(next_cell_state, scope='cell/'))
